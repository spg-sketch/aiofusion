...
**Project ownership is by creator.** Each `Client` in `aio.projects.v1` carries an optional `owner` (username) set at creation in `createStoredProject` from `getLocalSession()?.username`. User Management groups projects under each user via case-insensitive owner match. Pre-ownership projects are back-filled to the first admin by a one-time migration (`aio.projects.owner.migrated.v1`) that defers without setting its flag until an admin exists. **Why:** projects were originally a single shared global list with no user link; the chosen model is "your projects = the ones you created" rather than admin-assignment or a shared list. A project whose `owner` points to a deleted user simply stops appearing under any user (no reassignment flow exists).
