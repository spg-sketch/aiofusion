...
const signals: Signal[] = [
  {
    id: 1, title: "Schema Markup", icon: Code2, score: 8, rating: "green",
  ...
  {
    id: 2, title: "FAQ Page Structure", icon: HelpCircle, score: 4, rating: "amber",
  ...
  {
    id: 3, title: "Answer-First Copy", icon: MessageSquareQuote, score: 3, rating: "red",
  ...
  {
    id: 4, title: "AI Crawler Access", icon: Bot, score: 9, rating: "green",
  ...
  {
    id: 5, title: "Source Truth Consistency", icon: ShieldCheck, score: 6, rating: "amber",
  ...
  {
    id: 6, title: "Conversational Content", icon: MessagesSquare, score: 5, rating: "amber",
    ...
    <AppLayout currentPage="diagnostic">
      <div className="px-8 py-8 max-w-5xl mx-auto">
        <div className="flex items-start justify-between mb-8">
          ...
          <button className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium text-white transition-colors"
          ...
          <div className="p-6 flex items-center gap-8 rounded-t-xl" style={{ background: "linear-gradient(135deg, #f8fafc, #f1f5f9)" }}>
              ...
              <div className="relative">
                <svg width="120" height="120" viewBox="0 0 120 120">
                  ...
                  <circle cx="60" cy="60" r="52" fill="none"
                    ...
                    strokeWidth="8" strokeDasharray={`${(overallScore / 100) * 327} 327`}
                    strokeLinecap="round" transform="rotate(-90 60 60)" />
                </svg>
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  <span className="text-3xl font-bold" style={{ color: "var(--aio-navy)" }}>{overallScore}</span>
                  <span className="text-[10px] uppercase tracking-wider" style={{ color: "var(--aio-gray-400)" }}>out of 100</span>
                </div>
              </div>
              <p className="text-sm font-medium mt-3" style={{ color: "var(--aio-navy)" }}>Authority Index</p>
            </div>
            <div className="flex-1 grid grid-cols-3 gap-4">
              {(["green", "amber", "red"] as Rating[]).map((rating) => {
                ...
                return (
                  <div key={rating} className="rounded-lg p-3 border" style={{ background: "white", borderColor: "var(--aio-gray-200)" }}>
                    <div className="flex items-center gap-2 mb-1">
                      <config.icon size={16} color={config.color} />
                      <span className="text-xs font-medium" style={{ color: config.color }}>{config.label}</span>
                    </div>
                    <p className="text-2xl font-bold" style={{ color: "var(--aio-navy)" }}>{count}</p>
                    <p className="text-[11px]" style={{ color: "var(--aio-gray-400)" }}>signal{count !== 1 ? "s" : ""}</p>
                  </div>
                );
              })}
            </div>
        ...
        <div className="space-y-3 mb-8">
          {signals.map((signal) => {
            ...
            return (
              <div key={signal.id} className="rounded-xl border p-5 transition-all hover:shadow-sm"
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-sm mb-2" style={{ color: "var(--aio-gray-500)" }}>{signal.finding}</p>
                    <div className="flex items-start gap-2 p-3 rounded-lg" style={{ background: "var(--aio-gray-50)" }}>
                      ...
                      <p className="text-xs font-medium" style={{ color: "var(--aio-navy)" }}>{signal.action}</p>
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="rounded-xl border p-5 flex items-center justify-between"
          style={{ background: "rgba(79,143,255,0.03)", borderColor: "rgba(79,143,255,0.15)" }}>
          <div>
            <h3 className="text-sm font-semibold" style={{ color: "var(--aio-navy)" }}>Ready to improve your score?</h3>
