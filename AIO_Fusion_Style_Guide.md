...
| Project Set-Up | `#1A647B` solid teal — matches Dashboard's teal backdrop; all internal cards (AI-assist panel, track switcher, section body) remain white/pink-wash so they float on the teal, same pattern as Dashboard's metric cards. The AI-assist panel ("Add your company website") and the track switcher (PR Set-Up / AIO Set-Up) both use the card wash `#FBF1F0` (Dashboard secondary-card colour, see Section 3) rather than the badge pink `#FBE3ED`, so the two stacked cards read as one consistent dusky-pink surface. Header text sitting directly on the teal (page title, intro copy, "Save for later" button) switches to white/`rgba(255,255,255,0.85)`, matching how the Dashboard's heading/subtitle render on its teal background (see Section 4). The standalone "Sections" numbered nav box has been removed; "Project Data Actions" now lives as a footer row inside the section body card instead of its own strip. The old separate Previous/Save for later/Mark Complete/Next footer has also been removed — the section-progression button (Next / Continue to AIO Set-Up / Mark section complete) now sits inside the "Project Data Actions" footer, after "New Project", so there is a single consolidated footer per page. The outline "Save for later" button that used to sit at the top of the page (in the header row, next to the page title) has also been removed, since "Save for later" already lives in the bottom footer — the header now shows only the status badge. The "Project Data Actions" label (and its small pink bullet dot) that used to sit above the footer's action buttons has been removed — the footer now shows only the buttons themselves, no heading/eyebrow row. "Download these questions" and "How-to guide" now sit inside each section's own pink-wash header strip (top-right, alongside the "Section N / title / subtitle" block), rather than in the page's top header or under each section's intro text; both buttons keep the same pink outline styling (`#C8497A` text/border on white) so they read as a matched pair. In Section 1 (Earned Media) specifically, the intro copy and the "Use your agreed FAQs and approved company messaging" guidance note are consolidated into a single info box (still the pink-left-border card) above "Core Boilerplate", instead of two separate stacked boxes; "Use your agreed FAQs and approved company messaging." now renders as its own bold title line above the guidance body text, rather than as an inline bolded lead-in sentence. Other PR-track sections (FAQ & Customer Questions, Audience & Intent Mapping) keep the two-box layout unchanged. The PR Set-Up / AIO Set-Up track buttons have no persistent "active" look — both always rest at `#FBF1F0` and animate on hover to `#FBE3ED` with a `#C8497A` border (plus a lift/shadow), rather than the previously-selected track staying permanently highlighted. Each section's own header strip (the dark bar showing "Section N" + title + subtitle inside the white section-body card) now uses the same light `#FBF1F0` wash as the track-switcher cards above it, instead of solid dark navy `#102B36`; the title and subtitle text flip to dark navy (`#102B36` / `rgba(16,43,54,0.65)`) to stay legible on the light background, while the "Section N" eyebrow label stays pink (`#C8497A`) as before. |
...
### Subsection headers (grouping content within a card — e.g. Project Set-Up's "CORE BOILERPLATE")
- Mini version of the Section-header pink wash:
...
- **Scope: Project Set-Up (Intake Form) only.** This pink-wash treatment is reserved for the Project Set-Up page's field-grouping headings (e.g. "CORE BOILERPLATE"). Report, Diagnostic, Comms Planner, Marketing Intelligence and Media Research keep their original plain navy `h3` / accent-bar-only subsection headers — do not apply the pink-wash box there.
- Per-repeated-item eyebrow labels inside a loop (e.g. "Spokesperson 1", "Product / service 2", per-category "Findings"/"Recommendations") stay as plain small pink/navy uppercase text, never wrapped in the pink-wash bar, to avoid visual noise when the same label repeats many times.
...
- Simplified to 7 cards total (down from the earlier 6-card + trend-stat layout): a top row of 3 equal-weight cards — **Project Set-Up** (now first, showing the "X of Y sections complete" ring + checklist), **Earned Media Visibility Audit**, **Website Visibility Audit** — followed by a second row of 4 metric cards for **Total Articles** (now first), **In Planner** (Comms Planner item count), **In Draft** and **Final / Ready** (Archive counts). Both rows now share one consistent card format and size: every card is a `flex flex-col` container with a `min-h-[220px]` floor, an uppercase eyebrow label (with info tooltip) at the top, then the icon/number/content block, with the CTA button pinned to the bottom of the card via `mt-auto` — so the two rows read as a uniform grid rather than two differently-proportioned bands.
